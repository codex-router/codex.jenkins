...
3. **多选**：可以选择多个服务器以组合功能
4. **默认为空**：下拉列表默认为空 - 使用"更新 MCP 服务器列表"来填充
...
5. **MCP 服务器不可用**
   - **预期行为**：下拉列表默认为空 - 这是正常的。您必须点击"更新 MCP 服务器列表"来填充它。
...
7. **下拉列表中无可用模型**
   - **预期行为**：下拉列表默认为空 - 这是正常的。您必须点击"更新模型列表"来填充它。
   ...
   - 如果点击"更新模型列表"后仍没有模型出现，CLI 可能未正确配置或不可访问
